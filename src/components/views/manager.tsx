...
export const SessionManager: React.FC = () => {
  ...
  const handleSaveSession = async (sessionData: Partial<Session>) => {
    ...
    try {
      ...
      if (sessionData.id) {
        ...
        setSessions(sessions.map((s: Session) => (s.id === updated.id ? updated : s)));
  ...
  const handleSessionName = async (sessionData: Partial<Session>) => {
    ...
    const currentSession = sessions.find((s: Session) => s.id === sessionData.id);
  ...
  const chatViews = useMemo(() => {
    return sessions.map((s: Session) => {
      if (!s.id) return null; // Guard against undefined id
  ...
  return (
    <div className="relative flex flex-col h-full w-full">
      ...
      <div className="flex flex-1 relative">
        ...
        <div
          className={`flex-1 transition-all duration-200 min-w-0 overflow-auto bg-primary text-primary relative ${
          ...
          <div className="h-full">
            {activeSubMenuItem === "current_session" ? (
              session && sessions.length > 0 ? (
                <div className="pl-4 h-full">{chatViews}</div>
              ) : (
